...
(function(){
  ...
  if (set && typeof S !== "undefined" && S.admin && S.admin.isAdmin){
    S.admin.isAdmin = false; S.admin.pass = "";
...
function bootRender(){
  ...
  try {
    if (!routeHash()){
      /* Устройство взрослого открывается сразу в кабинет, а не в уроки: на нём
         занимаются не здесь, а дети — каждый на своём. */
      if (isAdminDevice()) screenAdminHome();
      else if (needsRegister()) screenRegister();   /* сервер настроен, ученик ещё не выбран */
      else screenWorlds();
    }
  } catch(e){
    bootFallback(e);
