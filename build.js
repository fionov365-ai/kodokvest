...
/* Комментарии CSS в сборке тоже снимаются: 80 КБ из 208 (замер 11.09.2026,
   1.147.0) — их никто не снимал, хотя у JS это делалось с 1.102.0. Разбор
   посимвольный, как у JS ниже: внутри кавычек знак комментария — просто
   текст. Отступы и пустые строки — тоже: переводов строки внутри строк CSS
   не бывает. Исходник css/style.css не трогается. */
function stripCssComments(src){
  ...
  while (i < n){
    ...
    if (c === "/" && src[i + 1] === "*"){
      ...
      if (j < 0) throw new Error("css/style.css: незакрытый комментарий");
      i = j + 2;
      continue;
    }
    if (c === '"' || c === "'"){
...
const css = stripCssComments(read("css/style.css"));
...
const scripts = ["js/engine-mini.js", "js/runtime.js", "js/curriculum.js",
                 ...
                 "js/cheatsheet.js", "js/specs.js", "js/algo.js", "js/algo-exam.js", "js/algo-ege.js", "js/algo-oge.js", "js/algo-thin.js", "js/robot.js", "js/robot-tasks.js", "js/web.js", "js/web-tasks.js", "js/exams.js", "js/fipi16.js", "js/fipi15.js", "js/parent.js", "js/homework.js",
                 "js/cloud-config.js", "js/cloud.js", "js/editor.js", "js/story.js", "js/studio.js", "js/sandbox.js", "js/account.js", "js/home.js", "js/screens-robot.js", "js/screens-web.js", "js/screens-defense.js", "js/screens-showcase.js", "js/screens-ailab.js", "js/screens-hw.js", "js/screens-viz.js", "js/screens-warm.js", "js/screens-today.js", "js/screens-shop.js", "js/certs.js", "js/screens-folio.js", "js/screens-mytasks.js", "js/screens-about.js", "js/screens-algo.js", "js/screens-specs.js", "js/screens-project.js", "js/variant.js", "js/proverka.js", "js/screens-zashchita.js"];
...
/* ============================================================
   ...
   ⚠️ 11.09.2026 (1.147.0) — второй раз о тот же потолок, и снова без подъёма
   порога. Сборка дошла до 3 108 303 байт при пороге 3 145 728: запас 36 КБ,
   то есть один-два пункта наполнения. Вес опять нашёлся в том, что не значит
   ничего: 149 КБ отступов в начале строк кода (снимаются здесь, в той же
   ветке кода, где схлопываются пустые строки) и 80 КБ комментариев в CSS,
   которые никто не снимал (stripCssComments выше). Отступы внутри строк и
   шаблонов — например, в программах на Python — не трогаются по устройству
   разбора: их копируют свои ветки.

   ⚠️ 24.09.2026 (1.215.0) — третий раз о потолок, и на этот раз его подняли:
   3 → 4 МиБ, решение фаундера. Запас был 7,7 КБ, а искать больше нечего —
   разбор 15.09.2026 показал, что в сборке лежит только содержимое. Число —
   в тесте [два контура] (ПОТОЛОК_МБ) и в tools/zapusk.js. */
const vm = require("vm");
/* Слова, после которых косая черта — начало регулярки, а не деление.
   Смотрим на уже собранный хвост: слово, потом пробелы, потом наша черта. */
const RE_AFTER_WORD =
  /(?:^|[^A-Za-z0-9_$.])(return|typeof|case|in|of|delete|void|instanceof|new|do|else|yield|await|throw)\s*$/;
function stripComments(src, name){
  ...
  while (i < n){
    ...
    if (c === "/" && d === "*"){
      ...
      i += 2;
      /* вместо блока — пробел, чтобы не склеить соседние слова. ⚠️ Но не в
         начале строки: там пробел стал бы новым отступом, и второй проход
         снял бы его — снятие перестало бы давать то же самое при повторе, а
         проверка [сборка] сравнивает тексты функций именно повторным снятием */
      if (out && !/\n$/.test(out)) out += " ";
      continue;
    }
    if (c === "'" || c === '"' || c === "`"){
    ...
    /* ⚠️ Регулярка начинается не только после знака, но и после КЛЮЧЕВОГО
       СЛОВА: `return /…/`, `typeof`, `case`, `in`. Пока слов здесь не было,
       такая регулярка считалась делением и копировалась посимвольно — а
       значит, первая же регулярка с `//` внутри (скажем, `return
       /^https:\/\//.test(u)`) была бы принята за комментарий, и хвост строки
       молча исчез бы из ОДНОФАЙЛОВОЙ сборки, оставив обычную работающей.
       Найдено ревизией 18.09.2026: девять таких мест в js/ уже есть, просто
       ни в одном пока нет двух слэшей подряд. */
    if (c === "/" && (/[(,=:[!&|?{};+\-*%~^<>]/.test(prev) || RE_AFTER_WORD.test(out))){
    ...
    /* Отступ в начале строки КОДА смысла не несёт — пропускаем его (1.147.0,
       замер 11.09.2026: 149 КБ). Только здесь, в ветке кода: строки, шаблоны
       и регулярки скопированы своими ветками выше целиком, поэтому отступы
       программ на Python внутри строк не трогаются.
       ⚠️ Пропуск стоит ПОСЛЕ обеих веток, и схлопнутой строки тоже. Первая
       версия выходила из схлопывания раньше пропуска, отступ следующей
       строки оставался, второй проход его снимал — и проверка [сборка]
       упала на пяти файлах: она сравнивает тексты функций повторным снятием.
       Снятие обязано давать то же самое при повторе. */
