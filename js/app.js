...
function lintRepeat(code, out){
  ...
  lines.forEach(function(ln, i){
    var t = ln.trim();
    if (t.length < 12 || t.charAt(0) === "#") return;
    if (!seen[t]) seen[t] = { n:0, line:i + 1 };
    seen[t].n++;
  });
  Object.keys(seen).forEach(function(t){
    if (seen[t].n < LINT_REPEAT_LINE) return;
    out.push({ line:seen[t].line, after:"for-range", rank:5,
      title:"Одна и та же строка " + seen[t].n + " раза",
      why:"Повторение — работа для цикла: то, что меняется, положи в список и пройди по нему циклом. " +
           "Тогда правка нужна в одном месте." });
  });
}
