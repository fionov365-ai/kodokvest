/* ============================================================
   ...
   проходами у самых концов стен и одно поле «очень больших длин» — стены по
   30 клеток и проходы длиннее 10 (js/robot-tasks.js). Ошибка, которая растёт
...
window.FIPI15 = (function(){
  ...
  function reasonHTML(g){
    ...
    if (g.score === 2) return "На всех " + g.total + " полях закрашено ровно нужное — " +
      "и на поле со стенами по 30 клеток.";
    ...
    return h + (g.score === 1
      ...
      : "Больше 10 — это 0: эксперт считает ошибку при очень длинных стенах и проходах, " +
        "а на нашем поле со стенами по 30 клеток она уже переросла порог.");
  }
