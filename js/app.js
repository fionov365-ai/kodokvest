...
function landShowcaseRender(i){
  ...
  box.innerHTML =
    ...
    '<div class="ldout" id="shout" aria-live="polite"></div>' +
    '<div class="dwbar"><button class="dwbtn go" data-sh="run">▶ Запустить заново</button>' +
      '<button class="dwbtn" data-sh="code">‹/› Показать код</button>' +
      '<span class="dwhint">' + rows + ' ' + plural(rows, "строка", "строки", "строк") +
      ', собранных за четыре урока</span></div>' +
    '<div class="ldcode" hidden><div class="dwcode">' + landCodeHTML(code) + '</div></div>';
  var out  = box.querySelector(".ldout"),
      cbox = box.querySelector(".ldcode"),
      cbtn = box.querySelector('[data-sh="code"]');
  box.querySelector('[data-sh="run"]').onclick = function(){ landRun(code, out, true); };
  cbtn.onclick = function(){
    cbox.hidden = !cbox.hidden;
    cbtn.textContent = cbox.hidden ? "‹/› Показать код" : "‹/› Спрятать код";
  };
