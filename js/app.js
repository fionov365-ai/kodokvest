...
function screenWorlds(){
  ...
  h += '<div class="sect"><h2>Моё</h2>' + qm("folio", "Что лежит в «Моём»") +
    ...
    '<span class="pjbody"><span class="pjkicker">покажи взрослым</span>' +
    '<b>Что создают ученики</b>' +
    '<span>шесть программ курса, рисунки и игры — всё запускается прямо там, ' +
    'это не картинки работ, а сами работы</span>' +
    '<span class="pjnote">Чужих детей и имён там нет: имя мы не спрашиваем вовсе.</span></span>' +
    '<button class="bigbtn ghost" id="goworks">Открыть витрину</button>' +
    '</div>';
  ...
  worldContent(n).then(function(){
    ...
    if (shelfN >= 3)
      h += '<div class="projcard"><span class="pjemoji">🔧</span>' +
        '<span class="pjbody"><span class="pjkicker">Мастерская · вне сотни уроков</span>' +
...
function screenShop(){
  ...
  var h = '<div class="lvlhead"><div><div class="idx">мастерская</div>' +
    ...
    '<p class="lede">Обычно решённая задача исчезает — и сто уроков превращаются в сто ' +
    'выброшенных вечеров. Здесь не так: каждая функция, которую ты написал сам, ' +
    'остаётся на полке. Из деталей собирается вещь, которой можно пользоваться.</p>';
