  ...
  SEEDS.forEach(seed => {
    ...
    if (JSON.stringify(again) !== JSON.stringify(built.vals))
      say(`[семя] ${id} (семя ${seed}): при повторном вычислении числа другие — ` +
          `у ребёнка и у репетитора разойдутся условия`);
  });
});
...
/* Покрытие: банк обязан доставать до КАЖДОГО мира, и не по одной задаче.
   ⚠️ Порог здесь не украшение. Пустой мир виден сразу, а мир с двумя задачами
   выглядит покрытым и молча ломает обещание: репетитор открывает выдачу
   домашки у ребёнка, дошедшего до середины, и находить ему нечего. Так и было
   с миром «Данные» — шесть задач на двадцать уроков, — пока их не добрали. */
const WORLD_MIN = 5;
{
  const worlds = {};
  BANK.forEach(it => {
  ...
  [1, 2, 3, 4, 5].forEach(w => {
    const n = worlds[w] || 0;
    if (!n) say(`[покрытие] в мире ${w} нет ни одной задачи-близнеца`);
    else if (n < WORLD_MIN)
      say(`[покрытие] в мире ${w} всего ${n} ${n % 10 === 1 && n !== 11 ? "задача" :
            (n % 10 > 1 && n % 10 < 5 && (n < 12 || n > 14) ? "задачи" : "задач")} — ` +
          `меньше ${WORLD_MIN}: репетитору нечего будет задать`);
  });
  console.log("Близнецов по мирам: " + [1,2,3,4,5].map(w => worlds[w] || 0).join(" / "));
}

/* Мир, закрытый ЦЕЛИКОМ: у каждого его урока есть свой близнец (1.144.0).
   ⚠️ Порог по числу здесь не помог бы: двадцать задач, из которых пять
   висят на одном уроке, выглядят покрытием, а репетитор, чей ученик
   застрял на «формах», всё равно найдёт пустоту. Поэтому проверяется не
   сколько, а ГДЕ: каждый урок мира назван в after хотя бы одной задачи.
   Мир 5 закрыт потому, что там все двадцать уроков — программы с
   вычислимым ответом, даже git (сравнение и слияние списков). Мир 3 закрыт
   12.09.2026 (1.156.0): классы, исключения, assert и dataclass движок
   выполняет, а «Свои модули» близнец обходит одним файлом, как мир 5
   обходит сеть словарём. В мирах 1–2 и 4 есть уроки без вычислимого
   ответа (черепашка, pip), и они сюда не входят. */
const FULL_WORLDS = [3, 5];
{
  const has = {};
  BANK.forEach(it => { has[it.after] = 1; });
  FULL_WORLDS.forEach(w => {
    const world = CURRICULUM.world(w);
    if (!world) return say(`[покрытие] мира ${w} в программе нет — поправь FULL_WORLDS`);
    const miss = world.lessons.filter(l => !has[l.id]).map(l => l.id);
    if (miss.length)
      say(`[покрытие] мир ${w} объявлен закрытым целиком, а у ${miss.length === 1 ? "урока" : "уроков"} ` +
          `${miss.join(", ")} близнеца нет — репетитору нечего задать после этого урока`);
  });
}
...
/* ── единственность верного ответа ──────────────────────────────────────
   ⚠ Проверка узкая по форме и широкая по смыслу. Условие «напечатай точку с
   самым большим y» имеет ОДИН верный ответ только пока максимум один. Когда
   наибольших значений два, верных ответов тоже два — а эталон печатает то,
   которое встретил первым, и ребёнок, написавший «>=» вместо «>», получает
   «неверно» за правильное решение.

   Такой брак не виден ни на одном из SEEDS: у hw-tochki он выпадал на шести
   значениях seedy из шестидесяти (4, 5, 24, 25, 44, 45), и нашёл его не тест,
   а ревью кода 08.09.2026. Поэтому здесь перебираются ВСЕ значения параметра,
   а не выборка семян — выборка ровно этот класс дефекта и пропускает.

   Заводя задачу, где ответ выбирается по «самому большому» или «самому
   маленькому», добавляй сюда строку. */
const UNIQUE_TOP = [
  { id: "hw-tochki", param: "seedy", what: "наибольший y",
    pick: v => v.pts.map(p => p[1]) },
];
UNIQUE_TOP.forEach(rule => {
  const item = BANK.find(x => x.id === rule.id);
  if (!item)
    return say(`[единственность] ${rule.id}: задачи с таким id больше нет — ` +
               `перенеси проверку на её замену или сними осознанно`);
  const prm = (item.params || []).find(x => x.k === rule.param);
  if (!prm)
    return say(`[единственность] ${rule.id}: нет параметра ${rule.param}`);
  const bad = [];
  for (let n = prm.min; n <= prm.max; n++){
    const v = {}; v[rule.param] = n;
    const vals = rule.pick(item.fix(v));
    const top = Math.max.apply(null, vals);
    if (vals.filter(x => x === top).length > 1) bad.push(n);
  }
  if (bad.length)
    say(`[единственность] ${rule.id}: ${rule.what} встречается дважды при ` +
        `${rule.param} = ${bad.join(", ")} — у задачи два верных ответа, ` +
        `а эталон печатает только первый`);
});
