...
/* ===== каждая механика «Ты и ИИ» — не меньше пяти упражнений (1.143.0) =====
   Решение 11.09.2026: механику из двух упражнений ребёнок проходит за пять
   минут и больше не встречает — умения из неё не остаётся. Порог тот же, что
   у тем экзамена. Финальный проект (boss) в счёт не идёт: он отдельная
   ступень «ответить за результат», а не ещё одно «почини». */
(function(){
  const МИН = 5, by = {};
  (global.AILAB || []).forEach(x => { if (!x.boss) by[x.type] = (by[x.type] || 0) + 1; });
  ["predict", "code", "fix", "review", "catch"].forEach(t => {
    if ((by[t] || 0) < МИН)
      say(`[ты-и-ии] механика «${t}»: ${by[t] || 0} упражнений — меньше ${МИН}, её пройдут за пять минут и забудут`);
  });
})();
