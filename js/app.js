...
function backTarget(){
  ...
  switch (curPlace){
    ...
    case "roles": case "parent": return null;
...
var PLACE_RU = {
  ...
  game:"играет", games:"выбирает игру", warmup:"делает разминку", warmups:"выбирает разминку",
  today:"на экране «Сегодня»", zan:"идёт занятие", again:"повторяет трудное",
  algo:"выбирает задачу экзамена", algoone:"решает задачу экзамена", ai:"в разделе «Ты и ИИ»",
  hw:"смотрит домашку", hwone:"делает домашку", sandbox:"в песочнице", viz:"в визуализаторе",
  mine:"составляет своё задание", friendtask:"решает задание от друга",
  play:"играет в игру по ссылке", specs:"в приёмке", folio:"в портфолио"
};
...
function presenceHTML(st, serverAt){
  ...
  return p ? '<div class="presence">🟢 Сейчас в тренажёре: <b>' + esc(p.what) + '</b></div>' : "";
}
...
function screenLiveView(code, label){
  ...
  function draw(rec){
    ...
    if (!box) return;
    if (!rec || !rec.found || (rec.now - rec.serverAt) > LIVE_FRESH){
    ...
    var age = Math.max(0, Math.round((rec.now - rec.serverAt) / 1000));
    box.innerHTML =
...
function kidRender(savedNote){
  ...
  h += '<div id="kidpresence">' + presenceHTML(st, kidTarget.serverAt) + '</div>';
  if (kidTarget.fresh)
...
function kidWatch(code, seq){
  ...
  function roll(){
    ...
    jobs.push(Cloud.liveGet(code).then(function(rec){
      return (rec && rec.found && (rec.now - rec.serverAt) <= LIVE_FRESH) ? rec : null;
    ...
    if (beat % 3 === 0)
      jobs.push(Cloud.load(code).then(function(r){
        ...
        if (r.found && r.data){
          kidTarget.serverAt = r.serverAt || 0;
          /* данные не подменяем: взрослый мог начать редактировать рамку */
          kidTarget.data.now = (ensureShape(r.data)).now;
        }
        return null;
      }, function(){ return null; }));
    Promise.all(jobs).then(function(res){
      ...
      if (el){
        el.innerHTML = presenceHTML(kidTarget.data, kidTarget.serverAt) +
          (live
...
window.__game = {
  ...
  presenceInfo: presenceInfo, presenceHTML: presenceHTML, PRESENCE_FRESH: PRESENCE_FRESH,
