...
(async function(){
  ...
  let pwaChecked = 0;
  {
    ...
    /* ⚠️ Потолок поднят 3 → 4 МиБ решением фаундера 24.09.2026 («поднимай
       потолок сборки»): запас сжался до 7,7 КБ, мусора в сборке не осталось
       (замер 15.09.2026, § 2.5 RAZVITIE), а два других пути — вынести уроки из
       файла или не наращивать тренажёр — хуже. Тот же потолок — ПОТОЛОК в
       tools/zapusk.js. Поднимать дальше — только словом фаундера (§ 7). */
    const ПОТОЛОК_МБ = 4;
    const distMb = fs.statSync(file).size / 1048576;
    if (distMb > ПОТОЛОК_МБ)
      bad(`[два контура] один файл вырос до ${distMb.toFixed(1)} МБ при потолке ${ПОТОЛОК_МБ} — обещание «лёгкий продукт» больше не правда`);
    const zapuskSrc = readRoot("tools/zapusk.js");
    if (!new RegExp("ПОТОЛОК = " + ПОТОЛОК_МБ * 1048576 + ";").test(zapuskSrc))
      bad("[два контура] потолок в tools/zapusk.js разошёлся с тестом — отчёт готовности считает запас не от того числа");
