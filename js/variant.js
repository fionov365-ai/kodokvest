...
function assignCardHTML(exId){
  ...
  var ex = examById(exId);
  /* ⚠️ Кто задал и кому — из самого назначения (13.09.2026). Раньше карточка
     всегда говорила «Репетитор» и «у всех в группе одинаковый»: с вариантом
     ОДНОМУ ученику (solo) и от родителя это стало бы неправдой. Назначения без
     поля by и solo — групповые от репетитора, как было всегда. */
  var who = t.by === "родитель" ? "Родитель" : "Репетитор";
  return '<div class="card"><h3>📮 ' + who + ' задал вариант</h3>' +
    ...
    '<p class="dim">' + (t.solo ? 'Вариант задан лично тебе — по коду он соберётся тот же на любом устройстве. '
                                : 'У всех в группе этот вариант одинаковый — по коду он собирается один и тот же. ') +
...
/* Карточка итога экзамена ОГЭ: балл за задание-программу по правилам
   эксперта. Пустая строка — карточки нет: не экзамен, не ОГЭ или модуля нет.
   ⚠️ Только у ЭКЗАМЕНА: в тренировке ответ сдают сколько угодно раз с
...
function programCardHTML(v){
  ...
  if (!x || !x.id) return "";
  var task = A.algoById(x.id);
  var h = '<div class="card"><h3>📝 Задание ' + x.n + ' — как оценил бы эксперт</h3>';
  if (!task || !F.applies(task)){
    return h + '<p>На этом номере в варианте стояла задача-функция — у неё нет ввода данных, ' +
      'поэтому по правилам эксперта ФИПИ её не оценить. Балла здесь нет, и выдумывать его ' +
      'мы не будем. В следующем варианте на этом номере будет программа с вводом.</p></div>';
  }
  var r = (v.pts || {})[x.n];
  if (!r)
    h += '<p><b>0 из ' + F.MAX + ' баллов</b> — ответ не сдан. На экзамене не сданное ' +
      'задание не оценивается вовсе.</p>';
  else {
    h += '<p><b>' + r.s + ' из ' + F.MAX + ' баллов</b>' +
      (r.z ? ' — ' + A.esc(F.zeroWhy(r.z)) :
        ' — программа верна на ' + r.p + ' ' + A.plural(r.p, "тесте", "тестах", "тестах") +
        ' из ' + r.t + '. 2 балла — верно на всех, 1 — неверно не больше чем на одном, 0 — иначе.') +
      '</p>';
    h += F.unpackWarnHTML(r);
  }
  return h + '<p class="dim">Правила — ' + A.esc(F.DOC.title) + ', ' + F.DOC.pages + '. ' +
    'Тесты эксперта ты не видел — как и на экзамене.</p></div>';
}
...
function screenVariantDone(){
  ...
  /* ---- задание-программа ОГЭ: балл так, как его ставит эксперт ----
     ⚠️ Решение шапки («первичного балла и перевода нет») здесь НЕ отменяется.
     Балл стоит за одно задание, и только потому, что его правила опубликованы
     целиком и применяются к одной программе: МР ФИПИ для экспертов ОГЭ-2026,
     с. 53–55 (js/fipi16.js). Складывать его с другими номерами нельзя — у
     тех своя проверка, и суммы, которую можно назвать, не существует. */
  var fipiRow = fipiCardHTML(v);
  h += fipiRow;
  ...
  h += '<div class="card"><h3>⚖️ Почему тут нет ' + (fipiRow ? 'общего балла' : 'баллов') + '</h3>' +
