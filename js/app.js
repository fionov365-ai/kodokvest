...
function parentSet(code){
  S.admin.parentOf = (typeof Cloud !== "undefined" && Cloud.validCode(code)) || "";
  saveLocal();
}
...
/* Превратить это устройство обратно в обычное. Нужна отдельная кнопка: иначе
   взрослый, поставивший админку по ошибке, окажется заперт без уроков. */
...
function screenAdminHome(){
  if (!isAdminDevice()) return screenAdminSetup();
...
function screenKidLogin(){
  ...
  document.getElementById("klnew").onclick = function(){ screenRegister(); };
...
function screenAdminLogin(){
  ...
  function go(){
    if (adminPassOk(inp.value)){ adminUnlock(); return screenKids(); }
    msg.className = "msg show bad";
...
function screenKids(){
  ...
  h += '<div class="pager"><button class="bigbtn ghost" data-kact="panel">Полная панель наставника →</button>' +
...
function screenParent(code){
  ...
  app.innerHTML =
    ...
    '<div id="kidbody"><p class="dim">Загружаю занятия с сервера…</p></div>' +
    '<div class="pager"><button class="bigbtn ghost" data-pd="off">Выйти</button></div>';
...
(function(){
  ...
  if (set && typeof S !== "undefined" && S.admin){
    if (S.admin.isAdmin){ S.admin.isAdmin = false; S.admin.pass = ""; try { adminLock(); } catch(e){} }
    S.admin.parentOf = "";               /* это устройство теперь детское, а не родительское */
...
(function(){
  ...
  /* родитель — не ученик и не админ на этом устройстве */
  S.admin.isAdmin = false; S.admin.pass = "";
...
window.__game = {
  ...
  isParentDevice: isParentDevice, parentOf: parentOf, parentSet: parentSet,
  parentDeviceOff: parentDeviceOff, parentLink: parentLink,
