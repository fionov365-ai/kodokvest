    ...
    b.onclick = function(){
      ...
      if (act === "bradd"){
        ...
        adultPick.breaksProblem =
          ...
          /* frameShape режет список до 12, и тринадцатая пауза раньше молча
             исчезала: добавилась и пропала, будто кнопка не сработала */
          (br.length >= 12) ? "Больше двенадцати пауз не помещается — уберите лишнюю, чтобы добавить новую." :
          null;
        if (adultPick.breaksProblem) return screenAdult();
        br.push([a, z]);
        frameSet({ breaks: br });
        return screenAdult();
      }
