...
function landTabsHTML(items, cls){
...
var LAND_ROLES = [
  { em:"👨‍👩‍👦", tab:"Родителю", how:"вход по коду ребёнка или по ссылке от учителя",
    lede:"Проверять код руками не нужно — всё проверил движок. Ваше дело посмотреть, " +
         "с кем поговорить.",
    items:[
  ...
  { em:"🛠", tab:"Администратору", how:"тому, кто ставит тренажёр себе, семье или кружку",
    ...
    items:[
      ...
      ["☁️","Свой сервер за полчаса","одна функция в Яндекс Облаке, один файл без единой библиотеки. Адрес правится в одном месте — `js/cloud-config.js`."],
...
function landWarmRender(i){
  ...
  box.innerHTML =
    '<div class="warmask"><div class="dwcode">' + landCodeHTML(w.code.replace(/\n$/, "")) + '</div>' +
      ...
      '<textarea id="warmin" rows="' + Math.min(5, want.split("\n").length) + '" spellcheck="false" ' +
