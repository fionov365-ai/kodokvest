...
function screenParent(code){
  ...
  app.querySelectorAll("[data-pd]").forEach(function(b){
    b.onclick = function(){
      ...
      if (!confirm("Выйти из кабинета родителя на этом устройстве?\n\n" +
                   "Ссылку от учителя можно открыть снова, чтобы вернуться."))
        return;
      kidTarget = null;
      parentDeviceOff();
      screenRoles();
...
function bootRender(){
  ...
  try {
    if (!routeHash()){
      ...
      if (isAdminDevice()) screenAdminHome();
      else if (isParentDevice()) screenParent();
      else if (myCode() || S.name) screenWorlds();
      /* ⚠️ Незакреплённое устройство встречает ВЫВЕСКА, а не выбор роли.
         Раньше здесь стоял screenRoles(), и первым, что видел человек по
         ссылке, были три двери без единого слова о том, куда они ведут. */
      else screenAbout();
    }
  } catch(e){
    bootFallback(e);
