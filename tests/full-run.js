...
(async function(){
  ...
  if (typeof g.screenFolio === "function"){
    ...
    if (ck !== "Курс пройден")
      bad("[сертификат] лист за курс называется «" + ck + "», а должен — «Курс пройден»");
  ...
  let pagesChecked = 0;
  {
    ...
    {
      const МЕНЮ = ["Индивидуальный проект", "Семейное обучение", "Репетитору",
                    "Школе", "Справочник", "О проекте"];
      html.forEach(rel => {
