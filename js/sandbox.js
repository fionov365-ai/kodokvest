...
/* ================= настоящий Python в песочнице =================
   ...
   устройства, как тема и звук (SFX_KEY, VOICE_KEY в app.js — здесь они не
   нужны и в контракт не передаются). На школьном компьютере
...
function screenSandbox(){
  ...
  document.getElementById("towork").onclick = function(){
    ...
    var r = Runtime.get(real ? "pyodide" : "mini").run(code, { stdin: [], interactive: true });
    /* ⚠️ Программа, которая спрашивает input(), НЕ сломана — она ждёт ответа.
       Пока это считалось ошибкой, «Угадай число» (её же предлагает подпись
       к полю!) сохранить было нельзя: прогон без ответов давал EOFError и
       ребёнок читал «Сначала пусть заработает» про работающую программу.
       Найдено ревизией 18.09.2026. Мини-движок в таком прогоне отвечает
       awaitingInput, настоящий Python — EOFError; принимаем и то и другое. */
    var ждётОтвета = r.awaitingInput || (r.error && r.error.kind === "EOFError");
    if (r.error && !ждётОтвета){
