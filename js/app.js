...
var HOME = KVSCREENS.home({
  ...
  hwPending: hwPending, certList: certList, partsList: partsList,
  buildsList: buildsList, galleryList: galleryList, myTasksList: myTasksList,
...
var SANDBOX = KVSCREENS.sandbox({
  ...
  screenFolio: screenFolio, screenStale: screenStale, screenViz: screenViz,
...
function examMapHTML(ex){
  ...
  ex.tasks.forEach(function(x){
    ...
    if (st === "yes"){
      ...
      right = '<span class="excnt">' + algoDoneIn(x.g) + ' из ' + algoCountIn(x.g) + '</span>' +
        '<span class="exwhy">кроме: ' + esc(x.off) + '</span>' +
        '<button class="rbtn sec" data-exgo="' + esc(x.g.join(",")) + '">Решать</button>';
    } else if (st === "no"){
      ...
      right = '<span class="exwhy">судить нечем: ' + esc(x.off) + '</span>';
    } else {
...
function screenAlgo(){
  ...
  if (algoTab !== "all"){
    ...
    app.innerHTML = h;
    app.querySelectorAll("[data-algotab]").forEach(function(b){
...
/* ---- взрослому: три роли вкладками ----
   ...
   репетитор и администратор — одним паролем в «Панель репетитора». Ролей при
...
var LAND_ROLES = [
  ...
  /* ⚠️ Третья вкладка — не третья дверь. Дверей в продукте две: код ребёнка
     и пароль. Пока здесь под ключиком стояло «тому, кто ставит тренажёр себе,
     семье или кружку», ключик обещал вход, а выдавал описание аудитории — и
     вывеска обещала дверь, которой нет. Теперь у всех трёх вкладок одно и то
     же устройство: КАК ВОЙТИ, а следом ЧЕМ ОТЛИЧАЮСЬ ОТ СОСЕДА. */
  { em:"🛠", tab:"Администратору", how:"вход по тому же паролю, что и у репетитора",
    win:"Кабинет · администратор",
    lede:"Не про учеников, а про устройство: замки, перенос, сервер.",
    tiles:[
      ["🔓","Снять замки","открыть все уроки"],
      ["♻️","Зачесть или сбросить","настройка устройства"],
      ["💾","Перенос файлом","когда сервера нет"],
      ["☁️","Свой сервер","полчаса, одна функция"],
      ["📵","Без сервера","работает полностью"]
    ] }
];
...
function lkMockHTML(title, tiles){
  return '<div class="lkmock"><div class="lkbar">' +
    '<span class="lkdots"><i></i><i></i><i></i></span><b>' + esc(title) + '</b>' +
...
function screenAbout(){
  ...
  var nums = [
    [c.lessons,  plural(c.lessons,  "урок", "урока", "уроков"),            "по порядку"],
    [c.worlds,   plural(c.worlds,   "мир", "мира", "миров"),               "по 20 уроков"],
    [c.projects, plural(c.projects, "проект", "проекта", "проектов"),      "большая программа"],
    [c.algo,     plural(c.algo,     "задача", "задачи", "задач"),
      "алгоритмы · ОГЭ " + c.oge + " · ЕГЭ " + c.ege],
    [c.warm,     plural(c.warm,     "разминка", "разминки", "разминок"),   "на пять минут"],
    [c.ai,       plural(c.ai,       "упражнение", "упражнения", "упражнений"), "принять работу ИИ"],
    [c.games,    plural(c.games,    "игра", "игры", "игр"),                "и у каждой виден код"],
    [c.cheat,    plural(c.cheat,    "команда", "команды", "команд"),       "шпаргалка поверх урока"]
  ];
  ...
  h += '<section class="landhero rv">' +
    ...
    '<div class="lhin"><div class="lhtext">' +
      ...
      /* ⚠️ Лид ровно в одну строку. Он читается ПОСЛЕ заголовка и ДО кнопки,
         ...
         не прочитал, а мы про это уже написали. Что видит взрослый — не здесь,
  ...
  var exE = examTally("ege"), exO = examTally("oge");
  if (exE && exO){
  ...
  h += '<div class="landsect rv">' +
    secHead("Чего нет у других", "Четыре вещи, ради которых это и делалось",
      "Не обещания — то, что уже работает.") +
    '<div class="bigs">' +

    '<div class="big"><span class="bigem">✈️</span>' +
      '<b>Интернет не нужен вообще</b>' +
      '<p>Python внутри свой. Весь курс — меньше двух мегабайт: ставится иконкой ' +
      'на домашний экран и открывается в самолёте, на даче и в метро.</p>' +
      ...
      '<p>И это не «почти как настоящий»: каждый пример сверяется с настоящим ' +
      '<b>python3</b> знак в знак при каждой сборке.</p>' +
    ...
    '<div class="big"><span class="bigem">🔎</span>' +
      ...
      '<p>Он видит, <b>как шла работа</b>: набирал сам или вставил готовое, сколько ' +
      'раз запускал, где споткнулся.</p>' +
      '<span class="bigwhy">Отсюда взрослый знает не «сделано», а «понял или списал».</span></div>' +

    '<div class="big"><span class="bigem">👀</span>' +
      '<b>Взрослый видит и настраивает всё</b>' +
      '<p>Видно, на каком уроке ребёнок <b>в эту минуту</b>. Дни, длина занятия, ' +
      'каникулы, потолок дня, дата экзамена — план тренажёр соберёт сам.</p>' +
      '<span class="bigwhy">Экран ребёнка — только с его согласия: он всегда видит, когда его видят.</span></div>' +

    '<div class="big"><span class="bigem">🔊</span>' +
      '<b>Читает вслух</b>' +
      '<p>Кнопка 🔊 у каждой карточки прочитает объяснение. А разбор ошибки может ' +
      'читаться сам — красный текст не надо разбирать глазами.</p>' +
      '<span class="bigwhy">Голос берётся у браузера: есть не везде, и это сказано в самом тренажёре.</span></div>' +
  ...
  h += '<section class="landend rv"><div class="lhaur" aria-hidden="true"></div>' +
    '<div class="endin"><div class="endleft">' +
      ...
      '<ul class="landhonest">' +
      '<li><b>Язык один — Python.</b> Ни Scratch, ни JavaScript.</li>' +
      '<li><b>Из экзамена — то, что решается программой.</b> ' +
        c.oge + ' в формате ОГЭ, ' + c.ege + ' в формате ЕГЭ, плюс кодирование, ' +
