  ...
  return '<div class="showcase" id="showcase">' +
    landTabsHTML(list.map(function(p){
      return { vars: landWorldVars(p.world),
               label: '<span class="lte">' + p.emoji + '</span>' +
                      '<span class="ltt"><i>Мир ' + p.world + '</i><b>' + esc(p.title) + '</b></span>' };
    }), "wld") +
...
function screenAbout(){
  ...
  h += '<div class="landsect rv">' +
    ...
    '<div class="big"><span class="bigem">✈️</span>' +
      ...
      'Весь курс со всеми уроками весит меньше двух мегабайт, ставится иконкой на ' +
      'домашний экран и открывается в самолёте, на даче и в метро. Прогресс копится ' +
      'на устройстве и догоняет сервер, когда сеть появится.</p>' +
      ...
      'прогоняется через настоящий <b>python3</b> и сверяется знак в знак. Разойтись ' +
      'незаметно наш Python не может — расхождение падает тестом у нас, а не всплывает ' +
      'у ребёнка.</p>' +
    ...
    '<div class="big"><span class="bigem">🔎</span>' +
      ...
      'готовое, сколько раз запускал, где споткнулся. Плюс проверка понимания — ребёнка ' +
      'просят предсказать, что напечатает <b>его собственная</b> программа. Само решение ' +
      'сверяет движок, в том числе скрытыми проверками, которых в условии не видно.</p>' +
      '<span class="bigwhy">Отсюда взрослый знает не «сделано», а «понял или списал».</span></div>' +

    '<div class="big"><span class="bigem">👀</span>' +
      '<b>Взрослый видит и настраивает всё</b>' +
      '<p>Видно прямо сейчас: открыт ли тренажёр и на каком уроке ребёнок в эту минуту. ' +
      'А в кабинете ставится рамка: дни недели, длина занятия 20/30/45 минут, ' +
      '<b>каникулы</b> отрезками дат, потолок экранного времени на день и дата, к которой ' +
      'надо успеть. Тренажёр сам собирает план под эту рамку.</p>' +
      '<span class="bigwhy">Экран ребёнка — только с его согласия: он всегда видит, когда его видят.</span></div>' +

    '<div class="big"><span class="bigem">🔊</span>' +
      '<b>Читает вслух</b>' +
      '<p>У каждой карточки урока есть кнопка 🔊 — она прочитает объяснение вслух. ' +
      'Отдельно включается автоматическое чтение: тогда разбор ошибки читается сам, ' +
      'и ребёнку не надо разбирать красный текст глазами. Звуковые сигналы синтезируются ' +
      'на лету, файлов нет вовсе — поэтому звук работает и без сети.</p>' +
      '<span class="bigwhy">Помогает тем, кто читает медленно, и младшим. Голос берётся у ' +
      'браузера — есть не везде, и это честно сказано в самом тренажёре.</span></div>' +
  ...
  h += '<div class="landsect rv">' +
    secHead("Курс целиком", "Программа: " + c.worlds + " " +
      ...
      "В конце каждого мира — босс, проект и сертификат.") +
    '<div class="landworlds">';
  CURRICULUM.forEach(function(w){
  ...
  /* --- витрина: что ребёнок соберёт сам. Вкладками, а не пятью карточками
         подряд, — и каждая программа тут же выполняется. --- */
  var showc = landShowcaseHTML();
  if (showc){
    h += '<div class="landsect rv">' +
      secHead("Что получается", "Пять программ, которые ребёнок унесёт с собой",
        "В конце каждого мира — проект: программа в несколько шагов, которую собирают " +
        "за четыре урока. Ниже не картинка результата, а сам результат: программа " +
        "выполняется прямо здесь, у вас в браузере.") + showc + '</div>';
  }
...
function landElseRender(i){
  ...
  if (i === 0){
    ...
    box.innerHTML = h + '</div>' +
      '<div class="instep"><div class="instepl">Каждый день — одна маленькая задача</div>' +
      '<ul class="rolelist">' +
      '<li><span class="rlem">🎯</span><span class="rltx"><b>Задача дня</b> — одна разминка ' +
        'на сегодня, до полуночи одна и та же и на планшете, и на компьютере. Берётся ' +
        'только из уже открытого: задачу на то, чего ещё не проходили, тренажёр не даст.</span></li>' +
      '<li><span class="rlem">🔥</span><span class="rltx"><b>Дни подряд и щит</b> — считаются ' +
        'дни, когда ребёнок занимался. Пропуск счёт не обнуляет: за каждые пять дней ' +
        'занятий копится щит, и он закрывает один пропущенный день. Щит срабатывает в ' +
        'тот момент, когда ребёнок вернулся, — он награждает возвращение, а не ' +
        'отсутствие.</span></li>' +
      '</ul></div>' +
      '<p class="ldpanefoot">Звёзд тут не дают и по порядку проходить не надо, ' +
