...
section("1. Шпаргалка ссылается на урок, где команды нет");
{
  ...
  /* И вторая половина того же вопроса: показывает ли пример то, что записано
     в заголовке. Запись «.startswith() и .endswith()» обещала две команды,
     а в примере была только вторая — ребёнок искал startswith и не находил.

     Кириллические имена в заголовке пропускаем: по соглашению справочника
     это ЗАГЛУШКИ («def имя(параметры)», «class Наследник(Родитель)»),
     а не настоящие названия. */
  let сверено = 0;
  const было = total;
  window.CHEATSHEET.forEach(g => g.items.forEach(it => {
    String(it.sig).replace(/[«»]/g, " ").split(/\s+/).forEach(кусок => {
      const m = /\.?([A-Za-z_][A-Za-z_0-9]*)\s*\(/.exec(кусок);
      if (!m) return;
      сверено++;
      if (String(it.code).indexOf(m[1]) < 0)
        hit(`${it.id} («${it.sig}»): в заголовке названа «${m[1]}», а в примере её нет`);
    });
  }));
  if (total === было) clean(сверено + " заголовков показаны примерами");
}
...
section("3. Первая подсказка содержит готовую строку решения");
{
  ...
  CURRICULUM.forEach(w => w.lessons.forEach(l => {
    ...
    /* Решение многофайлового задания лежит НЕ в task.solution: там main.py,
       ...
       хотя его первая подсказка выдавала обе функции модуля целиком. */
    const выдано = [String(b.task.starter || "")]
      .concat((b.task.files || []).map(f => f.starter || "")).join("\n");
    /* Считаем долю не от всего решения, а от того, что ребёнок ПРАВДА пишет:
       строки, которых нет в заготовке. Иначе длинная выданная обвязка
       (main.py, цикл с print внизу) разбавляет долю до нуля, и подсказка,
       выдающая половину настоящей работы, проходит как чистая. */
    const lines = [String(b.task.solution || "")]
      .concat((b.task.files || []).map(f => f.solution || "")).join("\n").split("\n")
      .map(x => x.trim())
      .filter(x => x.length > 12 && !x.startsWith("#") && выдано.indexOf(x) < 0);
    const найдено = lines.filter(x => first.indexOf(x) >= 0);
    /* Одна строка не должна считаться дважды. «return current» и
       «return current + amount» — две строки решения, но короткая целиком
       лежит внутри длинной, и подсказка с длинной засчитывала обе. */
    const leaked = найдено.filter(x => !найдено.some(y => y !== x && y.indexOf(x) === 0));
    /* Одна выданная строка из длинного решения — нормальный первый толчок.
       ...
       и звезда снимается сразу за всё. Порог и стоит на половине: пока доля
       считалась от всего решения вместе с выданной обвязкой, он был занижен
       до трети — чтобы хоть что-то ловить сквозь разбавление. Теперь
       знаменатель честный, и порог совпадает с тем, что написано словами. */
