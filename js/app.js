...
function mergeProgress(a, b){
  ...
  /* ===== заметки наставника к урокам (корзина 3.7) =====
...
/* ================= ПРИСУТСТВИЕ И ЖИВОЕ ЗАНЯТИЕ =================
   ...
   Уровень 2 — трансляция: наставник видит код и вывод ребёнка с задержкой
...
function liveBanner(showIt){
  ...
  el.innerHTML = '<span>🔴 <b>Наставник видит твой код</b></span>' +
...
function liveRowHTML(){
  ...
  if (liveShare.on)
    return '<div class="winrow"><button class="bigbtn ghost" id="zlive">🔴 Наставник видит твой код · выключить</button></div>';
  return '<div class="winrow"><button class="bigbtn ghost" id="zlive">📺 Показать экран наставнику</button></div>';
}
  ...
  function draw(raw){
    ...
    if (!rec){
      box.innerHTML = '<div class="note"><b>Трансляция сейчас не идёт</b>' +
        'Попросите ребёнка нажать «📺 Показать экран наставнику» — кнопка на экране занятия ' +
  ...
  worldContent(l.world).then(function(){
    ...
    app.innerHTML = head + howbar +
      '<div class="lessongrid' + (body.draw ? ' one' : '') + '">' +
        /* ⚠️ Заметка взрослого стоит ПЕРВОЙ, до теории. Если наставник
...
function noteWho(){ return (typeof hwWho === "function") ? hwWho() : "наставник"; }
...
function screenAccount(){
  ...
  if (serverOn() && code){
    h += '<div class="card"><h3>Что видят наставник и родитель</h3>' +
      '<ul class="trrules">' +
      '<li><b>Когда ты занимаешься и какой урок открыт</b> — по твоему коду ученика. ' +
      'Сам экран и то, что ты печатаешь, им не видно.</li>' +
      '<li><b>Твой код и вывод</b> — только если ты сам нажмёшь «Показать экран наставнику». ' +
  ...
  paintSound();
  (function(){ var r = document.getElementById("gorole"); if (r) r.onclick = screenRoles; })();
...
function screenAlgo(){
  ...
  h += '<div class="card"><h3>⚖️ Что мы обещаем, а что нет</h3><ul class="trrules">' +
    ...
    '<li><b>Номер задания не называем.</b> Нумерация между годами меняется, а сверить её ' +
    'по спецификации мы пока не смогли — она лежит архивом. Поэтому «в формате ОГЭ», а не «задание №».</li>' +
...
/* ================= экран: полная панель наставника =================
   Закрыта паролем взрослого (adminGate). Открывается адресом с #panel на
   конце — или ссылкой «Полная панель наставника →» из кабинета. Показывает
...
function clearAdminHash(){
  try {
    ...
    var hash = (hl === "#admin" || hl === "#panel") ? "" : location.hash;
...
var HASH_SCREENS = {
  ...
  /* Полная панель наставника (группа, просмотр по коду) — под своим адресом,
...
function wireLandDemo(){
  ...
  var ed = makeEditor(LAND_DEMO_OK, "попробуй поменять число лучей");
...
/* ---- вкладки на вывеске ----
   ...
   выбирать за читателя: родителю показывают ключ наставника, репетитору —
...
function landNums(code){
  var re = new RegExp(HLRE.source, "g"), m, out = [];
...
function landShowcaseRender(i){
  ...
  if (info && w){
    ...
    info.innerHTML = '<div class="landworld" style="' + landWorldVars(w.n) + '">' +
      ...
      '<div class="lwbody"><div class="lwkick">Мир ' + w.n + ' · ' + w.lessons.length + ' ' +
        plural(w.lessons.length, "урок", "урока", "уроков") +
        (boss ? ' · босс: ' + esc(boss.title) : "") + '</div>' +
      '<b>' + esc(w.title) + '</b>' +
      '<p>' + esc(w.desc) + '</p>' +
      '<div class="lwlist">' +
  ...
  box.innerHTML =
    '<div class="shhead"><span class="shico">' + p.emoji + '</span>' +
      ...
      '<b>' + esc(p.title) + '</b><p>' + esc(p.tagline || "") + '</p></div></div>' +
...
/* ---- взрослому: три роли вкладками ----
   ⚠️ Дверей в продукте ДВЕ, а не три: родитель входит по коду ребёнка,
   наставник и администратор — одним паролем в «Панель наставника». Ролей при
   этом три, потому что дела за одной дверью разные: репетитор смотрит группу,
   а администратор снимает замки и подключает сервер. Врать про третью дверь
   нельзя, поэтому у каждой вкладки прямо написано, как в неё попадают. */
var LAND_ROLES = [
  { em:"👨‍👩‍👦", tab:"Родителю", how:"вход по коду ребёнка или по ссылке от учителя",
    lede:"Видно, чем ребёнок занят прямо сейчас, и что было за неделю. Проверять код " +
         "руками не нужно — всё проверил движок; ваше дело посмотреть, с кем поговорить.",
    items:[
      /* ⚠️ Первой строкой — «сейчас», и это указание фаундера 06.09.2026:
         ...
         игры играет». Всё считается из обычного снимка прогресса, ребёнок
         ничего дополнительного не отправляет (presenceDetailHTML). */
      ["🟢","Видно прямо сейчас","открыт ли тренажёр и на каком уроке ребёнок сидит в эту минуту, что он уже сдал сегодня и идёт ли занятие. Строка обновляется сама, пока кабинет открыт."],
      ["📺","Его экран — только с его согласия","сам код и вывод показываются, когда ребёнок нажал «Показать экран». Подглядывать через плечо тренажёр не умеет и уметь не будет: он всегда показывает ребёнку, что его видят."],
      ["📅","Отчёт за неделю","сколько времени, какие уроки, где застрял. Точное время в минутах и секундах, а не «занимался / не занимался»."],
      ["💬","О чём спросить за ужином","готовый вопрос по тому, что ребёнок сегодня делал, — чтобы разговор был не «ну как, позанимался?»."],
      ["⏱","Занятие, а не поток уроков","20, 30 или 45 минут: тренажёр сам собирает план, зовёт на перерыв, сжимает план, если время уходит, и меряет, сколько занятие заняло на самом деле."],
      ["🗓","Расписание и потолок дня","в какие дни занимаемся и по сколько; сколько минут в день. Жёсткий запрет включается отдельно и никогда не обрывает начатый урок."],
      ["📝","Задание ребёнку","выбрать задачи и отдать их — он увидит их на Главном рядом с «Продолжить»."]
    ] },
  { em:"👨‍🏫", tab:"Репетитору и учителю", how:"вход по паролю, «Панель наставника»",
    demo:"Домашка, которую не спишешь: одна задача — у каждого свои числа. " +
         "Нажмите «другой ученик».",
    lede:"Всё, что есть у родителя, плюс группа. Это рабочее место, а не витрина: " +
         "оно отвечает на вопрос «с кем сегодня поговорить».",
    items:[
      ["👥","Группа","у каждого ученика своя ссылка и свой код. Видно, кто где идёт и кто затих, отчёты по всем сразу одной таблицей."],
      ["📺","Живое занятие","пока ребёнок занимается, видно, что происходит прямо сейчас — но только если он сам нажал «Показать экран наставнику». Скрытого наблюдения здесь нет."],
      ["📝","Домашка","банк задач-близнецов: у каждого свои числа, ответ считает движок. Выдаётся одному или всей группе, видно, кто сдал."],
      ["⚖️","Это не табель","«посмотреть в первую очередь» — приглашение поговорить, а не список отстающих. Вывод делаете вы, мы его за вас не выносим."],
      ["🔐","Имён детей на сервере нет","там код и результаты. Подпись «Петя, 5 класс» стоит у вас на устройстве и никуда не уходит."]
    ] },
  { em:"🛠", tab:"Администратору", how:"тому, кто ставит тренажёр себе, семье или кружку",
    lede:"Тренажёр — статический сайт: его можно просто открыть по ссылке, а можно " +
         "поставить себе целиком.",
    items:[
      ["🔓","Снять замки","открыть все уроки разом, если ребёнок идёт не с начала. Звёзды при этом не ставятся: решать он всё равно будет сам."],
      ["♻️","Зачесть или сбросить","отметить уже сделанное на три звезды или обнулить прогресс — это настройка устройства, на сервер она не уходит."],
      ["💾","Перенос файлом","прогресс выгружается и загружается файлом. Это запасной путь на случай, когда сервера нет вовсе."],
      ["☁️","Свой сервер за полчаса","одна функция в Яндекс Облаке, один файл без единой библиотеки. Адрес правится в одном месте — <code>js/cloud-config.js</code>."],
      ["📵","И без сервера тоже всё работает","без него тренажёр остаётся полностью рабочим: прогресс лежит в браузере, а сайт ставится иконкой на домашний экран и открывается без сети."]
    ] }
];
...
function landRolesRender(i){
  ...
  box.innerHTML =
    '<div class="rolein' + (r.demo ? "" : " one") + '"><div class="roleleft">' +
      '<div class="rolehow">🔑 ' + esc(r.how) + '</div>' +
      '<p class="rolelede">' + r.lede + '</p>' +
      '<ul class="rolelist">' + r.items.map(function(it){
        return '<li><span class="rlem">' + it[0] + '</span>' +
          '<span class="rltx"><b>' + it[1] + '</b> — ' + it[2] + '</span></li>';
      }).join("") + '</ul></div>' +
      /* Живой пример стоит только там, где он ЕСТЬ по-настоящему. Придумывать
         ...
         (в панели наставника имён детей нет). У репетитора же демонстрировать
...
function landWarmRender(i){
  ...
  box.querySelector("#warmgo").onclick = function(){
    ...
    if (mine === norm(want)){
      ...
      res.innerHTML = '<b>Сошлось ✓</b>' + esc(w.note || "");
      return;
    }
    ...
    res.innerHTML = '<b>Не сошлось</b>' +
      '<div class="warmdiff"><div><i>у тебя</i><pre>' + esc(mine) + '</pre></div>' +
      '<div><i>а печатает</i><pre>' + esc(want) + '</pre></div></div>' +
      '<span class="warmwhy">' + esc(w.note || "") + '</span>';
  };
  box.querySelector("#warmshow").onclick = function(){
    ta.value = want;
    res.className = "warmres show ok";
    res.innerHTML = '<b>Вот что печатает программа</b>' + esc(w.note || "");
  };
}
...
function landHwRender(){
  ...
  box.innerHTML =
    '<div class="hwtop"><span class="hwem">' + item.emoji + '</span>' +
      '<div><b>' + esc(b.title) + '</b><i>ученик №' + landHwSeed + '</i></div>' +
      '<button class="dwbtn" id="hwnext">🎲 Другой ученик</button></div>' +
    '<p class="hwgoal">' + esc(b.goal) + '</p>' +
...
function screenAbout(){
  ...
  h += '<section class="landhero rv">' +
    ...
    '<div class="lhin"><div class="lhtext">' +
      ...
      '<p class="landjump">Вы родитель, учитель или репетитор? ' +
      '<button class="linkjump" data-land="adults">Что вы получите со своей стороны →</button></p>' +
    '</div><div class="lhdemo">' + landDemoHTML() + '</div></div>' +
  '</section>';

  /* --- продукт в числах --- */
  var nums = [
    [c.lessons,  plural(c.lessons,  "урок", "урока", "уроков"),            "по порядку, от первой команды до своей программы"],
    [c.worlds,   plural(c.worlds,   "мир", "мира", "миров"),               "по двадцать уроков на одну большую тему"],
    [c.projects, plural(c.projects, "проект", "проекта", "проектов"),      "большая программа в несколько шагов"],
    [c.algo,     plural(c.algo,     "задача", "задачи", "задач"),
      "алгоритмы, " + c.oge + " в формате ОГЭ и " + c.ege + " в формате ЕГЭ"],
    [c.warm,     plural(c.warm,     "разминка", "разминки", "разминок"),   "пять минут, когда на урок нет сил"],
    [c.ai,       plural(c.ai,       "упражнение", "упражнения", "упражнений"), "как принимать работу у нейросети"],
  ...
  /* --- четыре больших блока: то, чего нет у конкурентов ---
         Заведены по указанию фаундера 06.09.2026: «надо выделить важное
         большими блоками, чтобы бросалось в глаза». Он прав в том, что до
         этого сильнейшие вещи продукта лежали строчками внутри карточек и
         вкладок — их читал только тот, кто и так уже заинтересовался.
         ⚠️ Отбор жёсткий: сюда идёт только то, чего у конкурентов НЕТ ни у
         кого (офлайн, доказательство авторства, живой присмотр, чтение
         вслух). Всё, что «как у всех», остаётся во вкладках ниже: большой
         блок под общее место обесценивает и остальные три. */
  h += '<div class="landsect rv">' +
    secHead("Чего нет у других", "Четыре вещи, ради которых это и делалось",
      "Курсов программирования для детей много, и почти всё в них устроено одинаково: " +
      "живой преподаватель, расписание, интернет. Здесь по-другому в четырёх местах, и " +
      "каждое из них — не обещание, а то, что уже работает.") +
    '<div class="bigs">' +

    '<div class="big"><span class="bigem">✈️</span>' +
      '<b>Интернет не нужен вообще</b>' +
      '<p>Python внутри свой: он не грузится из сети и не ставится на компьютер. ' +
      'Весь курс весит меньше двух мегабайт, ставится иконкой на домашний экран и ' +
      'открывается в самолёте, на даче и в метро.</p>' +
      /* ⚠️ Не «у нас неполный Python», а «у нас проверенный Python», и это не
         подмена слов. Родителю важен один вопрос — настоящему ли учат; ответ
         на него не «есть ли numpy» (в школе и на ЕГЭ его нет вовсе), а
         «расходится ли то, что ребёнок выучил, с настоящим языком». У нас на
         это отвечает тест при каждой сборке, и такого нет ни у кого.
         ⚠️ Число сверок сюда НЕ пишем: оно растёт с каждой новой задачей и
         через месяц врало бы. Тот же запрет, что на числа курса вообще. */
      '<p>И это не «почти как настоящий»: каждый пример курса при каждой сборке ' +
      'сверяется с настоящим <b>python3</b> знак в знак. Разойтись незаметно наш ' +
      'Python не может.</p>' +
      '<span class="bigwhy">У курсов с живым преподавателем без сети занятия просто нет.</span></div>' +

    '<div class="big"><span class="bigem">🔎</span>' +
      '<b>Тренажёр не верит на слово</b>' +
      '<p>Он записывает, <b>как шла работа</b>: сам ребёнок набирал решение или вставил ' +
      'готовое, сколько раз запускал, где споткнулся. И просит предсказать, что ' +
      'напечатает <b>его собственная</b> программа.</p>' +
      '<span class="bigwhy">Отсюда взрослый знает не «сделано», а «понял или списал».</span></div>' +

    '<div class="big"><span class="bigem">👀</span>' +
      '<b>Взрослый видит и настраивает всё</b>' +
      '<p>Видно прямо сейчас, на каком уроке ребёнок в эту минуту. А в кабинете ' +
      'ставится рамка: дни недели, длина занятия 20/30/45 минут, <b>каникулы</b>, ' +
      'потолок экранного времени и дата, к которой надо успеть. План тренажёр ' +
      'соберёт сам.</p>' +
      '<span class="bigwhy">Экран ребёнка — только с его согласия: он всегда видит, когда его видят.</span></div>' +

    '<div class="big"><span class="bigem">🔊</span>' +
      '<b>Читает вслух</b>' +
      '<p>Кнопка 🔊 у каждой карточки урока прочитает объяснение вслух. Отдельно ' +
      'включается автоматическое чтение: тогда разбор ошибки читается сам, и красный ' +
      'текст не надо разбирать глазами.</p>' +
      '<span class="bigwhy">Помогает тем, кто читает медленно, и младшим. Голос берётся ' +
      'у браузера — есть не везде, и это сказано в самом тренажёре.</span></div>' +
  ...
  h += '<div class="landsect rv">' +
    secHead("Изнутри", "Как это работает",
      "Урок — пятнадцать–двадцать минут внутри одной страницы. Ни установки Python, " +
      "ни редактора, ни командной строки. Две средние вкладки можно потрогать прямо сейчас: " +
      "там работает тот же движок, что и в уроке.") +
    landTabsHTML([
  ...
  /* --- взрослому: три роли вкладками. Раньше здесь стояли восемь одинаковых
         ...
         ключ наставника, репетитору — потолок экранного времени.
  ...
  h += '<div class="landsect rv" id="adults">' +
    secHead("Кабинет взрослого", "Взрослому",
      "У взрослого свой кабинет, отдельно от тренажёра ребёнка: ребёнок в него не " +
      "заходит и ничего в нём не меняет. Выберите, кто вы, — покажем только ваше.") +
    landRolesHTML() + '</div>';
  ...
  if (showc){
    h += '<div class="landsect rv">' +
      secHead("Курс целиком", "Программа: " + c.worlds + " " +
        ...
        plural(c.lessons, "урок", "урока", "уроков"),
        "Уроки идут по порядку: следующий открывается, когда сдан предыдущий. " +
        "В конце каждого мира — босс, проект и сертификат. Выберите мир: под ним — " +
        "программа, которую ребёнок соберёт в конце, и она выполняется прямо здесь.") +
      showc + '</div>';
  }
  ...
  h += '<div class="landsect rv">' +
    secHead("Кроме уроков", "Между делом и на память",
      "Сотня уроков — не всё. Слева то, куда заходят по настроению, справа — то, " +
      "что ребёнок унесёт с собой на бумаге и файлами.") +
    landTabsHTML([
  ...
  h += '<section class="landend rv"><div class="lhaur" aria-hidden="true"></div>' +
    '<div class="endin"><div class="endleft">' +
      ...
      '<ul class="landhonest">' +
      '<li><b>Язык один — Python.</b> Ни Scratch, ни HTML с JavaScript. Даже урок про сайт ' +
        'в пятом мире пишется на Python.</li>' +
      '<li><b>Экзамены закрываем не целиком.</b> Берём то, что решается программой: ' +
        'алгоритмику, кодирование информации, логику и графы. <b>Электронные таблицы сюда ' +
        'не входят</b> — их нечем проверить: наш судья умеет запускать Python, а не считать ' +
        'формулы Excel.</li>' +
      '<li><b>Проверяет программа, а не человек.</b> Живого учителя и оценок тут нет: ' +
        'решение сверяет движок, а объясняет сам урок.</li>' +
      '<li><b>Прогресс лежит в браузере.</b> Чтобы он был виден на втором устройстве ' +
        'и в кабинете взрослого, нужен код ученика.</li>' +
      '</ul></div>' +
    '<div class="endright"><span class="seckick">Вход</span><h2>С чего начать</h2>' +
      ...
      '</div>' +
      '<p class="landsub"><b>Сегодня доступ бесплатный.</b> Это не акция и не пробный период: ' +
      'платить тут негде — оплаты в продукте нет вовсе, он ещё наполняется. Появится платная ' +
      'часть — будет написано здесь же.</p></div></div></section>';

  h += '</div>';
  app.innerHTML = h;

  app.querySelectorAll("[data-land]").forEach(function(b){
    b.onclick = function(){
...
function landHowRender(i){
  ...
  if (i === 0){
    box.innerHTML =
      '<ol class="landsteps">' +
      '<li><b>Читаешь объяснение</b><span>Короткая теория, а к ней живые примеры: у каждого ' +
        'кнопка «▶ Запустить пример» — код срабатывает тут же, на месте.</span></li>' +
      '<li><b>Решаешь задачу</b><span>Задача стоит списком требований, а не одной фразой: ' +
        'видно, что именно должна делать программа.</span></li>' +
      '<li><b>Запускаешь свой код</b><span>Сколько угодно раз, это ничего не стоит. Рядом со ' +
        'строками написано, что каждая из них сделала.</span></li>' +
      '<li><b>Жмёшь «✓ Проверить»</b><span>Не сошлось — тренажёр покажет, чем твой ответ ' +
        'отличается от нужного. Подсказки идут по шагам: от намёка к ответу.</span></li>' +
      '</ol>' +
      ...
      '<div class="instep"><div class="instepl">И ещё две вещи прямо в уроке</div>' +
      '<ul class="rolelist">' +
      '<li><span class="rlem">💬</span><span class="rltx"><b>Спроси</b> — четыре кнопки ' +
        'рядом с задачей: «Что тут нового?», «Что надо сделать?», «Почему ошибка?», ' +
        '«Покажи пример». Тренажёр отвечает текстом и читает ответ вслух. Ни микрофона, ' +
        'ни нейросети: он отвечает только про этот урок и решить задачу за ребёнка ' +
        'не может — именно поэтому так и сделано.</span></li>' +
      '<li><span class="rlem">🧹</span><span class="rltx"><b>Ревью кода</b> — кнопка под ' +
        'редактором. Программа уже работает, а тренажёр разбирает её по дереву и говорит, ' +
        'что можно написать чище, и объясняет почему. Советует только про то, что уже ' +
        'проходили, и звёзд за это не отнимает.</span></li>' +
      '</ul></div>' +
      '<div class="note"><b>Ошибка здесь — не ругань</b>Красный текст Python переведён на ' +
      'русский: написано, в какой строке и что именно не сошлось. А если код работает не так, ' +
      'как думал, — визуализатор пройдёт программу по шагам и покажет, что лежит в памяти.</div>';
    return;
  }
  if (i === 1){
  ...
  box.innerHTML =
    ...
    '<div class="landcard dir" style="' + landWorldVars(3) + '">' +
      ...
      '<p>Поиск, сортировка, системы счисления, кодирование информации, логика и графы — ' +
      'и типовые задания обоих экзаменов: <b>' + c.oge + '</b> в формате ОГЭ и <b>' + c.ege +
      '</b> в формате ЕГЭ. Плюс то, чего нет нигде: цену алгоритма тут не рассказывают, ' +
      'а считают шагами — «837 шагов против 11» видно числом на экране.</p></div>' +
    '<div class="landcard dir" style="' + landWorldVars(4) + '">' +
      ...
      ' не про то, как попросить нейросеть, а про то, как принять её работу: найти, где машина ' +
      'уверенно врёт, и доказать это проверкой. Рядом — <b>' + c.specs + '</b> ' +
      plural(c.specs, "работа", "работы", "работ") + ' на приёмку.</p></div>' +
    '<div class="landcard dir" style="' + landWorldVars(5) + '">' +
      ...
      'Собранное остаётся у ребёнка.</p></div>' +
    '</div>' +
    '<div class="note"><b>Про экзамены — без обещаний, которых мы не сдержим</b>' +
    'Мы закрываем то, что решается программой, и с 1.77.0 это уже не только алгоритмика: ' +
    'добавлены кодирование информации, логика и графы. Электронные таблицы не закрываем — ' +
    'их нечем проверить. Номера заданий не называем: нумерация между годами меняется.</div>';
}
...
function landElseRender(i){
  ...
  if (i === 0){
    ...
    trainCards().filter(function(t){ return t.id !== "ai" && t.id !== "algo"; }).forEach(function(t){
      h += '<div class="landcard"><span class="lcem">' + t.em + '</span>' +
        '<b>' + esc(t.title) + '</b><p>' + esc(t.why) + '</p></div>';
    });
    ...
    box.innerHTML = h + '</div>' +
      '<p class="ldpanefoot">Каждый день тренажёр предлагает одну маленькую разминку — ' +
      'только из уже пройденного. Звёзд тут не дают и по порядку проходить не надо, ' +
      'но день занятий засчитывается и здесь.</p>';
    return;
  }
  box.innerHTML = '<div class="landgrid g3">' +
    '<div class="landcard"><span class="lcem">🎒</span><b>Портфолио и сертификаты</b>' +
      ...
      ' — за каждый мир, за разделы и за курс целиком. Печатаются на бумагу или в PDF.</p></div>' +
    '<div class="landcard"><span class="lcem">🖼</span><b>Рисунки и файлы .py</b>' +
      '<p>Что нарисовала черепашка — уезжает в галерею. Любую свою программу можно выгрузить ' +
      'настоящим файлом .py и открыть во взрослом Python.</p></div>' +
    '<div class="landcard"><span class="lcem">✍️</span><b>Своё задание</b>' +
      '<p>Ребёнок сам придумывает задачу, движок считает по ней правильный ответ, а задание ' +
      'уезжает ссылкой другу — или взрослому: пусть теперь он решает.</p></div>' +
    '<div class="landcard"><span class="lcem">🎮</span><b>Своя игра по ссылке</b>' +
      '<p>Собранную игру отправляют одной ссылкой. Тот, кому её прислали, играет сразу ' +
      'в браузере и может заглянуть в код — ничего не устанавливая.</p></div>' +
    '<div class="landcard"><span class="lcem">🧰</span><b>Мастерская</b>' +
      '<p>Куски своих программ складываются на полку деталей, а на верстаке из них собирается ' +
      'новая — своя, а не по заданию.</p></div>' +
    '<div class="landcard"><span class="lcem">🔁</span><b>Повторить</b>' +
      '<p>Уроки, которые дались тяжело, возвращаются сами через несколько дней. Рядом — ' +
      'бестиарий: каждая ошибка, которую ребёнок победил, с её именем.</p></div>' +
    '</div>';
}
...
function screenRoles(){
  ...
  app.innerHTML =
    ...
    '<h1>🐍 Кодоквест — кто занимается?</h1></div></div>' +
    ...
    '<div class="rolegrid">' +
      ...
      '<button class="rolecard" data-role="admin"><span class="roleico">🛠</span>' +
        '<b>Я администратор</b><span class="dim">Ученики, ссылки, отчёты. Вход по паролю.</span></button>' +
    '</div>' +
...
function screenKids(){
  ...
  var h = '<div class="lvlhead"><div><div class="idx">кабинет взрослого</div>' +
    ...
    '<p class="lede">Каждому ученику — своя ссылка и своё устройство. Здесь вы ставите расписание ' +
    'и смотрите, как идут занятия. Ребёнок вашего кабинета не видит.</p>';

  if (!serverOn())
    h += '<p class="warnline">⚠️ Сервер не подключён: в файле <code>js/cloud-config.js</code> не указан адрес. ' +
         'Без него ученики не смогут заниматься на своих устройствах, а вы — видеть их занятия.</p>';

  h += '<div class="card"><h3>Добавить ученика</h3>' +
    '<p class="dim">Впишите имя — тренажёр придумает код и даст ссылку. Ссылку отправьте ребёнку: ' +
    'открыв её на своём устройстве, он сразу окажется в тренажёре под своим именем.</p>' +
    '<div class="admgate">' +
  ...
  if (!kids.length){
    ...
    h += '<div class="kidlist">' + kids.map(function(k){
  ...
  /* Полная панель наставника — «под капотом»: группа, просмотр любого ученика
     по коду, список всех с сервера. Обычному родителю с парой детей она не
     нужна, репетитору с классом — да, поэтому лежит отдельной ссылкой. */
  h += '<div class="pager">' +
    /* ⚠️ Выход стоит ПЕРВЫМ и отдельно от «больше не кабинет»: это разные
       вещи, и раньше лёгкого выхода не было вовсе. */
...
function screenParent(code){
  ...
  app.innerHTML =
    ...
    '<div id="kidbody"><p class="dim">Загружаю занятия с сервера…</p></div>' +
    '<div class="pager">' +
    '<button class="bigbtn ghost" data-pd="leave">🚪 Выйти из кабинета</button>' +
    '<span class="sp"></span><button class="bigbtn ghost" data-pd="role">⇄ Сменить роль</button>' +
    /* ⚠️ Кнопка переименована. Она забывает код ребёнка на устройстве — это
       не «выйти», а «отвязать», и рядом с новым лёгким выходом старое слово
       «Выйти» означало бы две разные вещи одним словом. */
    '<span class="sp"></span><button class="bigbtn ghost" data-pd="off">Забыть код на этом устройстве</button></div>' +
    aboutFootHTML();
  wireAboutFoot(app);
  app.querySelectorAll("[data-pd]").forEach(function(b){
    b.onclick = function(){
      if (b.getAttribute("data-pd") === "leave") return leaveRoom();
      if (b.getAttribute("data-pd") === "role"){ kidTarget = null; return screenRoles(); }
      if (b.getAttribute("data-pd") !== "off") return;
      if (!confirm("Забыть код ребёнка на этом устройстве?\n\n" +
                   "Кабинет закроется совсем. Ссылку от учителя можно открыть снова, " +
                   "чтобы вернуться."))
        return;
      kidTarget = null;
      parentDeviceOff();
...
/* ===== отчёт родителю: текст для мессенджера =====
   ...
   доставки — сам наставник: скопировал, вставил в чат, подписал имя.

   Три правила текста, и все три нарушать нельзя:
     — имени ребёнка в тексте НЕТ. Наставник подписывает сам: наш текст обязан
...
function parentReportCardHTML(){
  /* Карточку видят двое, и слова у каждого свои: наставник ШЛЁТ этот текст
...
/* ===== о чём спросить на занятии =====
   Готовит наставника к разговору за минуту: три вопроса из недели ученика.
...
/* Порог тишины — 3 дня, был 5. «Не сел вовсе» — самая частая и самая дорогая
   подмена, и пять дней означали, что наставник узнаёт о ней к следующему
...
function groupRow(code, st, serverAt){
  ...
  /* ⚠️ Затык — ПЕРЕД «тяжело шёл урок». Тот про уже сданный урок, то есть про
     ...
     и это единственная пометка, по которой наставнику надо действовать
...
/* ===== напоминание молчащему =====
   ...
   наставнике: сочинить сообщение. Теперь текст готов — скопировал и отправил
...
/* ===== домашка всей группе одной кнопкой =====
   ...
   переносятся без ослабления:
     — каждый получает ТОЛЬКО задачи по пройденным ИМ урокам. «Всей группе»
       ...
       приедет, и это видно наставнику числом, а не выясняется у ребёнка;
...
function groupAssignHW(ids, due){
  ...
  function next(){
    ...
    return Cloud.load(row.code).then(function(r){
      ...
      mine.forEach(function(id){
        ...
        if (base.hw[key]) return;
        base.hw[key] = { id:id, seed:seed, due:due || "", by:"наставник",
                         at: Date.now(), done:0, tries:0 };
        added++;
      });
  ...
  function finish(){
    ...
    /* Итог обязан называть и то, что НЕ уехало: наставник, уверенный, что
       задал всем, хуже наставника, который знает, что троим не дошло. */
...
var HELP = {
  ...
  about: { t:"🐍 Кодоквест — что это", h:
    ...
    '<li><b>«Начать заниматься»</b> спросит, кто вы: ученик, родитель или администратор.</li>' +
