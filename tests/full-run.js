...
(async function(){
  ...
  /* --- 11б. редактор на вывеске не растягивается --- */
  {
    ...
    if (typeof g.LAND_DEMO_OK === "string"){
      ...
      if (!/#\s*←/.test(g.LAND_DEMO_OK))
        bad("[вывеска] в примере не показано стрелкой, какое число менять");
      const перваяСтрока = g.LAND_DEMO_OK.split("\n")[0];
      if (!/#/.test(перваяСтрока) || !/\d/.test(перваяСтрока.split("#")[0]))
        bad("[вывеска] стрелка стоит не на строке с числом: " + перваяСтрока);
