...
(async function(){
  ...
  if (typeof g.screenAbout === "function" && typeof g.screenKids === "function"){
    ...
    /* ⚠️ Лид раздела стоит ПОД заголовком, а не отдельной колонкой справа.
       ...
       уровне ВТОРОЙ строки заголовка — «как плевок, брошенный почти по
       центру» (фаундер, 08.09.2026). Раскладку в jsdom не измерить, поэтому
       стережём правило: у шапки раздела нет колонок. */
    {
      ...
      if (!/\.secthead \.lede\{[^}]*max-width/.test(cssH))
        bad("[вывеска] у лида раздела нет потолка ширины — длинная строка растянется во всю страницу");
    }
