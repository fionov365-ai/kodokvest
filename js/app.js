...
function zanStart(){
  ...
  var rec = { start: Date.now(), end: 0, len: frame().len, plan: zanPlanFor(key),
              done: [], sec: 0, pause: 0, predOk: 0, predAll: 0 };
...
function zanNote(kind, id, extra){
  ...
  save();
  /* всё по плану сделано — занятие закрывается само */
  if (rec.done.length >= rec.plan.length) zanFinish("plan");
}
...
function zanReport(rec, st){
  ...
  if (rec.done.length < rec.plan.length) was += " — занятие закончили раньше плана";
  ...
  return { was: was, praise: praise, got: got, ask: ask,
           mins: mins, pause: pause, lessons: lessons.length,
           full: rec.done.length >= rec.plan.length };
}
...
function zanCardHTML(){
  ...
  if (open){
    var pct = Math.min(100, Math.round((open.done.length / Math.max(1, open.plan.length)) * 100));
    return '<div class="card zancard on"><h3>⏱ Занятие идёт</h3>' +
      '<div class="zanbar"><i style="width:' + pct + '%"></i></div>' +
      '<p>Сделано ' + open.done.length + ' из ' + open.plan.length + '. Работы: ' +
...
function screenToday(){
  ...
  app.innerHTML =
    ...
    installTipHTML() + saved + banner + zanCardHTML() + ptaskCardHTML() + hero + taskCard + schedBox +
    '<div class="pager"><button class="bigbtn ghost" id="tomap">← На главную</button></div>';
...
function screenZan(){
  ...
  var next = null;
  rec.plan.forEach(function(b){ if (!next && !doneSet[b.k + ":" + b.id]) next = b; });
  var mins = zanMins(rec), pause = zanPauseMins(rec);
  var pct = Math.min(100, Math.round((rec.done.length / Math.max(1, rec.plan.length)) * 100));

  app.innerHTML =
    '<div class="lvlhead"><div><div class="idx">идёт занятие</div>' +
      '<h1>⏱ Занятие на ' + rec.len + ' минут</h1></div>' +
      '<div class="right"><span class="tag">' + rec.done.length + ' из ' + rec.plan.length + '</span></div></div>' +
    '<div class="zanbar"><i style="width:' + pct + '%"></i></div>' +
    '<p class="zanmeta">Работы: <b>' + mins + ' ' + plural(mins, "минута", "минуты", "минут") + '</b>' +
      (pause >= 2 ? ' · перерыв: <b>' + pause + '</b>' : '') +
      (zanTimeUp(rec) ? ' · <b>время вышло — можно заканчивать</b>' : '') + '</p>' +
    '<div class="card"><h3>Шаги занятия</h3><ol class="zanplan">' +
      rec.plan.map(function(b){
        var done = !!doneSet[b.k + ":" + b.id];
        var cur = !done && next && next.id === b.id && next.k === b.k;
        return '<li class="' + (done ? "done" : (cur ? "cur" : "")) + '">' +
          '<span class="zi">' + (done ? "✓" : zanBlockEmoji(b)) + '</span>' +
          '<b>' + zanBlockLabel(b) + '</b> · ' + esc(b.title || b.id) +
          (cur ? ' <button class="rbtn check zopen" data-zk="' + b.k + '" data-zi="' + esc(b.id) + '">Открыть</button>' : '') +
        '</li>';
      }).join("") + '</ol></div>' +
    '<div class="winrow">' +
  ...
  document.getElementById("zend").onclick = function(){
    var fin = zanFinish("hand");
    screenZanDone(fin || rec);
  };
  refreshTop();
}
