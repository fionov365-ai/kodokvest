...
/* ===== возвращаемость по СВОЕМУ списку (24.09.2026) =====
   ⚠️ КОПИЯ счёта op=stats из cloud/index.js, строка в строку: облачная функция
   ничего из игры не видит, игра — из облака. Молча разойтись они не могут:
   тест [возвращаемость] прогоняет одни и те же снимки через обе и требует
   совпадения до цифры. Зачем копия: ключ сервера (ADMIN_KEY) один на всех и
   открывает ВСЕХ детей — репетитору его не дают, а свои цифры ему нужны.
   Считаем по снимкам, которые «Группа» уже загрузила по кодам его учеников. */
function groupStatsCalc(list, now){
  var DAY = 864e5, dayMs = function(k){ return Date.parse(k + "T12:00:00Z"); };
  var priceOf = function(g){ return ((g && g.attempts) || 0) + ((g && g.hints) || 0) * 2 + ((g && g.shown) ? 5 : 0); };
  var byLesson = {}, REACH = [20, 40, 60, 80, 100], reached = REACH.map(function(){ return 0; });
  var students = 0, started = 0, weekEligible = 0, weekReturned = 0, active4w = 0, recent = [];
  function rec(id){ return byLesson[id] || (byLesson[id] = { lesson: id, tried:0, solved:0, stuck:0, attemptsSum:0 }); }
  (list || []).forEach(function(data){
    data = data || {}; students++;
    var lg = data.log || {}, sm = data.stars || {}, seen = {};
    Object.keys(lg).forEach(function(k){ seen[k] = 1; });
    Object.keys(sm).forEach(function(k){ seen[k] = 1; });
    Object.keys(seen).forEach(function(id){
      var g = lg[id] || {}, done = sm[id] !== undefined;
      if (!done && !(g.attempts > 0)) return;
      var it = rec(id); it.tried++;
      if (done){ it.solved++; return; }
      if (priceOf(g) < STUCK_PRICE) return;
      it.stuck++; it.attemptsSum += (g.attempts || 0);
    });
    var days = Object.keys(data.days || {}).sort();
    if (!days.length) return;
    started++;
    var first = dayMs(days[0]);
    if (now - first >= 8 * DAY){
      weekEligible++;
      if (days.some(function(k){ var d = (dayMs(k) - first) / DAY; return d >= 1 && d <= 7; })) weekReturned++;
    }
    var solvedN = Object.keys(sm).length;
    REACH.forEach(function(n, i){ if (solvedN >= n) reached[i]++; });
    var l28 = 0;
    Object.keys(lg).forEach(function(k){ var g = lg[k] || {}; if (g.solvedAt && now - g.solvedAt <= 28 * DAY) l28++; });
    if (days.some(function(k){ return now - dayMs(k) <= 28 * DAY; })){ active4w++; recent.push(l28); }
  });
  var stuck = Object.keys(byLesson).map(function(k){ return byLesson[k]; })
    .filter(function(it){ return it.stuck > 0; })
    .sort(function(a, b){ return (b.stuck - a.stuck) || (b.tried - a.tried) ||
      (a.lesson < b.lesson ? -1 : a.lesson > b.lesson ? 1 : 0); })
    .slice(0, 12)
    .map(function(it){ return { lesson: it.lesson, tried: it.tried, solved: it.solved, stuck: it.stuck,
      avgAttempts: Math.round(it.attemptsSum / it.stuck) }; });
  recent.sort(function(a, b){ return a - b; });
  var median = recent.length ? (recent.length % 2 ? recent[(recent.length - 1) / 2]
    : (recent[recent.length / 2 - 1] + recent[recent.length / 2]) / 2) : 0;
  return { students: students, started: started,
    week: { eligible: weekEligible, returned: weekReturned },
    reach: REACH.map(function(n, i){ return { lessons: n, students: reached[i] }; }),
    month: { active: active4w, medianLessons: median }, stuck: stuck };
}

function groupStatsHTML(){
  var h = '<div class="card"><h3>📈 Возвращаемость</h3>' +
    '<p class="dim">Две главные цифры: вернулся ли ребёнок в первую неделю ' +
    'и доходит ли до конца мира. ' + (grpStats.data && !grpStats.data.local
      ? 'Посчитано по ключу сервера — по всем ученикам на сервере, не только по вашим.'
      : 'Считается по вашим ученикам из списка; ключ сервера для этого не нужен.') + '</p>';
  if (grpStats.error)
    h += '<div class="msg show bad"><b>Не получилось</b>' + esc(grpStats.error) + '</div>';
  var m = grpStats.data;
  if (m){
    h += '<ul class="trsum">' +
      '<li>' + (m.local ? 'Учеников с занятиями на сервере' : 'Кодов на сервере') + ': <b>' + m.students +
        '</b>, начинали заниматься: <b>' + m.started + '</b>.</li>';
    if (m.week.eligible)
...
function bindGroupStats(){
  ...
  btn.onclick = function(){
    ...
    /* Без ключа — свои ученики, счёт здесь же (groupStatsCalc). С ключом — все
       на сервере, как раньше: это цифры владельца продукта. */
    if (!key){
      if (!kidsList().length){ grpStats.error = "Список учеников пуст — заведите их на вкладке «Ученики»."; return screenGroup(); }
      grpStats.busy = true; grpStats.error = ""; screenGroup();
      var ready = (groupState.rows && groupState.src === "list") ? Promise.resolve() : groupLoad("");
      return ready.then(function(){
        grpStats.busy = false;
        grpStats.data = groupStatsCalc(groupState.datas || [], Date.now());
        grpStats.data.local = true;
        screenGroup();
      }, function(err){
...
function groupLoad(key){
  ...
  groupState.datas = []; groupState.src = key ? "server" : "list";   /* сырые снимки — для возвращаемости */
    ...
    function next(){
      ...
      return Cloud.load(code).then(function(res){
        if (res && res.found && res.data){
          groupState.datas.push(JSON.parse(JSON.stringify(res.data)));
          rows.push(groupRow(code, ensureShape(res.data), res.serverAt || 0));
        }
...
var HELP = {
  ...
  about: { t:"🐍 Фионика — что это", h:
    ...
    /* ⚠️ Правка 20.09.2026 по вопросу фаундера «зачем эта кнопка». Кнопка «?»
       нужна (обещание index.html: она отвечает на ЛЮБОМ экране), а вот текст
       здесь устарел: он вёл к кнопке «Начать заниматься», которой на вывеске
       нет с 1.138.0. Подсказка, зовущая нажать несуществующее, хуже молчания. */
    '<li><b>«Войти по коду»</b> — для тех, кто уже занимался: код ученика ' +
    'открывает прогресс на любом устройстве. Взрослому — та же дверь: по коду ' +
    'ребёнка виден его кабинет.</li>' +
    '<li>Строка <b>«Кто вы»</b> — ученик, родитель, репетитор или учитель: ' +
    'ведёт туда, где написано про вас.</li>' +
...
window.__game = {
  ...
  groupStatsCalc: groupStatsCalc, grpStats: grpStats, stuckTopHTML: stuckTopHTML,
